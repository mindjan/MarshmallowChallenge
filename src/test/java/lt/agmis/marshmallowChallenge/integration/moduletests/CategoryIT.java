package lt.agmis.marshmallowChallenge.integration.moduletests;
...
import lt.agmis.marshmallowChallenge.integration.testutils.MockRequest;
...
import static lt.agmis.marshmallowChallenge.integration.matchers.IsPositiveInteger.positiveInteger;
...
public class CategoryIT extends AbstractTestNGSpringContextTests
{
	...
	public void setUp() throws Exception {
		mockMvc = webAppContextSetup(wac)
				.defaultRequest(get("/").contextPath("/marshmallowChallenge").servletPath("/api"))
				.build();
		mockRequest = new lt.agmis.marshmallowChallenge.integration.testutils.MockRequest(mockMvc);
    ...
    public static final Object[][] getUnsuportedHttpMethodsForCategoryWs() {
        return new Object[][] {
                {"categories PUT method",
                        put("/marshmallowChallenge/api/categories")},
                {"categories DELETE method",
                        delete("/marshmallowChallenge/api/categories")},
                {"categories/id GET method",
                        get("/marshmallowChallenge/api/categories/{id}", 1)},
                {"categories/id POST method",
                        post("/marshmallowChallenge/api/categories/{id}", 1)}};
    }
    ...
    @Test(dependsOnMethods = {"categoryWsShouldRejectUnsuportedHttpMethodsWith404"})
     public void shouldReturnZeroCategoriesWhenNoCategoriesWereCreated() throws Exception {
        String json =  mockRequest.retrieveJson(get("/marshmallowChallenge/api/categories"));
    ...
    public void creationOfCategoryShouldReturnId() throws Exception {
        String json = mockRequest.submitJsonRetrieveJson(post("/marshmallowChallenge/api/categories"), "{\"name\":\"Programming Languages Category\"}");
    ...
    public void shouldCreateAndRetrieveCategory() throws Exception {
        String jsonWithCreatedCategoryId = mockRequest.submitJsonRetrieveJson(post("/marshmallowChallenge/api/categories"), "{\"name\":\"Programming Languages Category\"}");
        Integer categoryId = JsonPath.read(jsonWithCreatedCategoryId, "$.id");
        String jsonCategoryList = mockRequest.retrieveJson(get("/marshmallowChallenge/api/categories"));
    ...
    public void shouldUpdateCategory() throws Exception {
        String jsonWithCreatedCategoryId = mockRequest.submitJsonRetrieveJson(post("/marshmallowChallenge/api/categories"), "{\"name\":\"Programming Languages Category\"}");
        Integer categoryId = JsonPath.read(jsonWithCreatedCategoryId, "$.id");
        mockRequest.submitJson(put("/marshmallowChallenge/api/categories/{id}", categoryId), "{\"name\":\"Updated Category\"}");
        String jsonCategoryList = mockRequest.retrieveJson(get("/marshmallowChallenge/api/categories"));
        String jsonCategory = extractCategoryJsonFromListJsonWithId(jsonCategoryList, categoryId);
        with(jsonCategory)
                .assertThat("$.id", equalTo(categoryId))
    ...
    public void shouldDeleteCategory() throws Exception {
        String jsonWithCreatedCategoryId = mockRequest.submitJsonRetrieveJson(post("/marshmallowChallenge/api/categories"), "{\"name\":\"Programming Languages Category\"}");
        Integer categoryId = JsonPath.read(jsonWithCreatedCategoryId, "$.id");
        mockRequest.submitRequest(delete("/marshmallowChallenge/api/categories/{id}", categoryId));
        String jsonCategoryList = mockRequest.retrieveJson(get("/marshmallowChallenge/api/categories"));
